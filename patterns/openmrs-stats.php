<?php
/**
 * OpenMRS Stats Pattern with Multiple Color Schemes
 *
 * @package OpenMRS Theme
 */

// Base content for the stats pattern (teal)
$stats_content_teal = '<!-- wp:group {"tagName":"section","className":"section_layout289 background-color-dark-teal"} -->
<section class="wp-block-group section_layout289 background-color-dark-teal">
    <!-- wp:group {"className":"padding-global padding-section-large"} -->
    <div class="wp-block-group padding-global padding-section-large">
        <!-- wp:group {"className":"container-large"} -->
        <div class="wp-block-group container-large">
            <!-- wp:group {"className":"layout289_component"} -->
            <div class="wp-block-group layout289_component">
                <!-- wp:columns {"className":"layout289_list w-layout-grid"} -->
                <div class="wp-block-columns layout289_list w-layout-grid">
                    <!-- wp:column {"className":"layout289_item"} -->
                    <div class="wp-block-column layout289_item">
                        <!-- wp:group {"className":"margin-bottom margin-small"} -->
                        <div class="wp-block-group margin-bottom margin-small">
                            <!-- wp:group {"className":"layout289_item-icon-wrapper"} -->
                            <div class="wp-block-group layout289_item-icon-wrapper">
                                <!-- wp:image {"sizeSlug":"full","linkDestination":"none","className":"is-style-icon-code"} -->
                                <figure class="wp-block-image size-full is-style-icon-code"><img src="data:image/svg+xml;base64," alt="Code Icon"/></figure>
                                <!-- /wp:image -->
                            </div>
                            <!-- /wp:group -->
                        </div>
                        <!-- /wp:group -->
                        <!-- wp:group {"className":"margin-bottom margin-xsmall"} -->
                        <div class="wp-block-group margin-bottom margin-xsmall">
                            <!-- wp:heading {"level":3,"className":"heading-style-h5 text-color-white"} -->
                            <h3 class="wp-block-heading heading-style-h5 text-color-white">8,100+</h3>
                            <!-- /wp:heading -->
                        </div>
                        <!-- /wp:group -->
                        <!-- wp:paragraph {"className":"text-color-white"} -->
                        <p class="text-color-white">code commits in 2024</p>
                        <!-- /wp:paragraph -->
                    </div>
                    <!-- /wp:column -->
                    <!-- wp:column {"className":"layout289_item"} -->
                    <div class="wp-block-column layout289_item">
                        <!-- wp:group {"className":"margin-bottom margin-small"} -->
                        <div class="wp-block-group margin-bottom margin-small">
                            <!-- wp:group {"className":"layout289_item-icon-wrapper"} -->
                            <div class="wp-block-group layout289_item-icon-wrapper">
                                <!-- wp:image {"sizeSlug":"full","linkDestination":"none","className":"is-style-icon-contributors"} -->
                                <figure class="wp-block-image size-full is-style-icon-contributors"><img src="data:image/svg+xml;base64,..." alt="Contributors Icon"/></figure>
                                <!-- /wp:image -->
                            </div>
                            <!-- /wp:group -->
                        </div>
                        <!-- /wp:group -->
                        <!-- wp:group {"className":"margin-bottom margin-xsmall"} -->
                        <div class="wp-block-group margin-bottom margin-xsmall">
                            <!-- wp:heading {"level":3,"className":"heading-style-h5 text-color-white"} -->
                            <h3 class="wp-block-heading heading-style-h5 text-color-white">1000</h3>
                            <!-- /wp:heading -->
                        </div>
                        <!-- /wp:group -->
                        <!-- wp:paragraph {"className":"text-color-white"} -->
                        <p class="text-color-white">individual contributors</p>
                        <!-- /wp:paragraph -->
                    </div>
                    <!-- /wp:column -->
                    <!-- wp:column {"className":"layout289_item"} -->
                    <div class="wp-block-column layout289_item">
                        <!-- wp:group {"className":"margin-bottom margin-small"} -->
                        <div class="wp-block-group margin-bottom margin-small">
                            <!-- wp:group {"className":"layout289_item-icon-wrapper"} -->
                            <div class="wp-block-group layout289_item-icon-wrapper">
                                <!-- wp:image {"sizeSlug":"full","linkDestination":"none","className":"is-style-icon-globe"} -->
                                <figure class="wp-block-image size-full is-style-icon-globe"><img src="data:image/svg+xml;base64,..." alt="Globe Icon"/></figure>
                                <!-- /wp:image -->
                            </div>
                            <!-- /wp:group -->
                        </div>
                        <!-- /wp:group -->
                        <!-- wp:group {"className":"margin-bottom margin-xsmall"} -->
                        <div class="wp-block-group margin-bottom margin-xsmall">
                            <!-- wp:heading {"level":3,"className":"heading-style-h5 text-color-white"} -->
                            <h3 class="wp-block-heading heading-style-h5 text-color-white">80+</h3>
                            <!-- /wp:heading -->
                        </div>
                        <!-- /wp:group -->
                        <!-- wp:paragraph {"className":"text-color-white"} -->
                        <p class="text-color-white">countries</p>
                        <!-- /wp:paragraph -->
                    </div>
                    <!-- /wp:column -->
                </div>
                <!-- /wp:columns -->
            </div>
            <!-- /wp:group -->
        </div>
        <!-- /wp:group -->
    </div>
    <!-- /wp:group -->
</section>
<!-- /wp:group -->';

// Purple and Orange variants
$stats_content_purple = str_replace('background-color-dark-teal', 'background-color-dark-purple', $stats_content_teal);
$stats_content_orange = str_replace('background-color-dark-teal', 'background-color-dark-orange', $stats_content_teal);

// Register patterns
register_block_pattern('openmrs-theme/stats-teal', array(
    'title'         => __('Stats (Teal)', 'openmrs-theme'),
    'description'   => __('A section displaying statistics with icons (teal background)', 'openmrs-theme'),
    'categories'    => array('openmrs-theme'),
    'keywords'      => array('statistics', 'icons', 'numbers', 'teal'),
    'viewportWidth' => 1200,
    'content'       => $stats_content_teal,
));

register_block_pattern('openmrs-theme/stats-purple', array(
    'title'         => __('Stats (Purple)', 'openmrs-theme'),
    'description'   => __('A section displaying statistics with icons (purple background)', 'openmrs-theme'),
    'categories'    => array('openmrs-theme'),
    'keywords'      => array('statistics', 'icons', 'numbers', 'purple'),
    'viewportWidth' => 1200,
    'content'       => $stats_content_purple,
));

register_block_pattern('openmrs-theme/stats-orange', array(
    'title'         => __('Stats (Orange)', 'openmrs-theme'),
    'description'   => __('A section displaying statistics with icons (orange background)', 'openmrs-theme'),
    'categories'    => array('openmrs-theme'),
    'keywords'      => array('statistics', 'icons', 'numbers', 'orange'),
    'viewportWidth' => 1200,
    'content'       => $stats_content_orange,
));